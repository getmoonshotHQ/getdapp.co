...
<script>
  ...
  dapps.forEach(dapp => {
    if (CATEGORIES.indexOf(dapp.category) < 0) {
      CATEGORIES.push(dapp.category);
    }
  });
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Latest & Top Blockchain Dapps powered by Elastos | Learn Everything
  ...
  <meta
    property="og:title"
    content="Discover your next favorite decentralized app!" />
  <meta
  ...
  <meta
    ...
    content="https://www.getdapp.co/social_share.jpg" />
</svelte:head>
{#if currentdApp}
  <DappPopup dapp={currentdApp} close={handleClose} />
{/if}
...
<TitledSection title="Featured DApps" noborder nopadding>
  <HorizontalSlider>
    {#each featuredDapps as dapp}
      <FeaturedDapp onDappPicked={handleOpen} {dapp} />
    {/each}
  </HorizontalSlider>
</TitledSection>
    ...
    <TitledSection title={category.charAt(0).toUpperCase() + category.slice(1)}>
      <DappsArray>
        {#each dAppsByCategory[category] as dapp}
          <Dapp
            title={dapp.appName}
            iconSrc={`https://dapp-store.elastos.org/apps/${dapp.packageName}/icon`}
            on:click={evt => {
              handleOpen(dapp);
            }} />
        {/each}
